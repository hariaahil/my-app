...
const games = [
  { name: "Skyfall Arena", description: "Explore a social 3D island, invite friends and play together in a shared room.", href: "/games/skyfall-arena", featured: true },
  ...
  return (
    <main id="main-content" className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <header className="max-w-2xl">
        ...
        <h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">Play, compete and come back</h1>
        <p className="mt-3 text-sm leading-6 text-black/60">Quick browser games plus a social 3D multiplayer experience built for TargetBud.</p>
      </header>
          ...
          {games.map((game) => (
            <a key={game.href} href={game.href} className={`rounded-2xl border p-5 transition hover:-translate-y-0.5 hover:shadow-sm focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-black ${game.featured ? "border-black bg-black text-white sm:col-span-2" : "border-black/10"}`}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div><p className={`text-[10px] font-bold uppercase tracking-[0.16em] ${game.featured ? "text-white/50" : "text-black/40"}`}>{game.featured ? "Featured · Multiplayer 3D" : "TargetBud Games"}</p><h3 className="mt-2 text-lg font-black">{game.name}</h3><p className={`mt-2 text-sm leading-6 ${game.featured ? "text-white/65" : "text-black/55"}`}>{game.description}</p></div>
                {game.featured && <span className="rounded-full border border-white/20 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-white/70">Invite friends</span>}
              </div>
