...
import { Film, Music2, Pause, Play, Plus, RotateCcw, Scissors, SkipBack, SkipForward, Trash2, Type, Upload, Volume2 } from "lucide-react";
...
export default function VideoStudio() {
  ...
  const pushHistory = (next: Clip[]) => setHistory(h => [...h.slice(-19), clips]);
  ...
  const splitSelected = () => {
    if (!selectedClip || selectedClip.duration < 2) return;
  ...
  const exportPreview = async () => {
    ...
    try {
      ...
      a.download = "targetbud-video-preview.webm";
      a.click();
  ...
  return <main className="min-h-[calc(100vh-56px)] bg-[#f7f7f7] text-black">
    ...
    <div className="mx-auto grid max-w-7xl gap-4 px-4 py-4 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)_260px] lg:px-8">
      ...
      <aside className="space-y-4"><div className="rounded-2xl border border-black/10 bg-white p-4"><p className="text-[11px] font-bold uppercase tracking-[.16em] text-black/40">Quick edits</p><label className="mt-4 block text-xs font-bold">Text overlay<input value={text} onChange={e=>setText(e.target.value)} placeholder="Add a title…" className="mt-2 w-full rounded-xl border border-black/10 px-3 py-2.5 text-sm outline-none focus:border-black"/></label><label className="mt-4 block text-xs font-bold">Speed <span className="float-right font-normal text-black/45">{speed}×</span><input type="range" min="0.25" max="2" step="0.25" value={speed} onChange={e=>setSpeed(Number(e.target.value))} className="mt-3 w-full"/></label><label className="mt-4 block text-xs font-bold"><span className="inline-flex items-center gap-1"><Volume2 size={13}/> Volume</span><input type="range" min="0" max="1" step="0.05" value={volume} onChange={e=>setVolume(Number(e.target.value))} className="mt-3 w-full"/></label></div><div className="rounded-2xl border border-black/10 bg-white p-4"><p className="text-[11px] font-bold uppercase tracking-[.16em] text-black/40">Coming next</p><ul className="mt-3 space-y-2 text-xs leading-5 text-black/55"><li>• Caption generation</li><li>• Transitions & filters</li><li>• MP4 export pipeline</li><li>• Templates for Shorts/Reels</li><li>• News/article → short video</li></ul></div></aside>
    </div>
  </main>;
}
