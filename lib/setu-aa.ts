...
export async function setuAccessToken() {
  const response = await fetch(SETU_TOKEN_URL, {
    ...
    body: JSON.stringify({
      clientID: required("SETU_AA_CLIENT_ID"),
      secret: required("SETU_AA_CLIENT_SECRET"),
...
async function setuFetch(path: string, init: RequestInit = {}) {
  ...
  const response = await fetch(`${SETU_BASE_URL}${path}`, {
    ...
    headers: {
      ...
      "x-product-instance-id": required("SETU_AA_PRODUCT_INSTANCE_ID"),
      ...(init.headers || {}),
    },
    cache: "no-store",
  });
  ...
  if (!response.ok) {
    const message = typeof body?.errorMsg === "string" ? body.errorMsg : `Setu request failed (${response.status})`;
...
export async function getSetuAccountAvailability(mobileNumber: string) {
  return setuFetch("/v2/account-availability", {
    method: "POST",
...
export async function getSetuConsent(consentId: string) {
  return setuFetch(`/v2/consents/${encodeURIComponent(consentId)}?expanded=true`, { method: "GET" });
}

export async function createSetuDataSession(consentId: string, from: string, to: string) {
  return setuFetch("/sessions", {
    method: "POST",
    body: JSON.stringify({ consentId, dataRange: { from, to }, format: "json" }),
  });
}
