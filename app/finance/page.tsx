...
export default function FinancePage() {
  ...
  async function importCsv(event: ChangeEvent<HTMLInputElement>) {
    ...
    setImporting(true); setError(""); setMessage(""); setMobileMenuOpen(false);
    try {
  ...
  return <main className="min-h-screen px-5 py-10"><div className="mx-auto max-w-6xl"><div className="flex items-center justify-between gap-4"><div><p className="text-sm text-violet-300">PRIVATE WORKSPACE</p><h1 className="mt-2 text-4xl font-semibold">My Finance</h1><p className="mt-2 text-slate-400">Signed in as {userEmail}</p></div><div className="flex items-center gap-2"><button onClick={() => setMobileMenuOpen((open) => !open)} aria-label="Open finance menu" aria-expanded={mobileMenuOpen} className="rounded-xl border border-white/15 px-3 py-2 text-xl leading-none hover:bg-white/10 sm:hidden">☰</button><button onClick={signOut} className="rounded-xl border border-white/15 px-4 py-2 text-sm hover:bg-white/10">Sign out</button></div></div>
    {mobileMenuOpen && <div className="mt-4 rounded-2xl border border-white/10 bg-slate-950/95 p-3 shadow-xl sm:hidden"><p className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-slate-500">Finance menu</p><label className="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-3 font-semibold hover:bg-white/10"><span className="text-lg">📥</span><span>{importing ? "Analyzing…" : "Import transactions"}</span><input type="file" accept=".csv,text/csv" onChange={importCsv} disabled={importing} className="hidden" /></label><p className="px-3 pb-2 text-xs text-slate-500">GPay CSV • smart categorization • duplicate protection</p></div>}
    <section className="mt-8 hidden rounded-3xl border border-violet-400/20 bg-violet-500/5 p-6 sm:block"><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-xl font-semibold">Smart Transaction Import</h2><p className="mt-1 text-sm text-slate-400">Upload your GPay CSV. TargetBud detects food, fuel, shopping, bills, transport and more.</p></div><label className="cursor-pointer rounded-xl bg-violet-500 px-5 py-3 font-semibold hover:bg-violet-400">{importing ? "Analyzing…" : "Upload GPay CSV"}<input type="file" accept=".csv,text/csv" onChange={importCsv} disabled={importing} className="hidden" /></label></div>{error && <p className="mt-4 text-sm text-red-300">{error}</p>}{message && <p className="mt-4 text-sm text-emerald-300">{message}</p>}</section>
