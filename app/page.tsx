...
import { ArrowUpRight, BarChart3, Bell, BookOpen, Calculator, Film, Search, Trophy, WalletCards, CloudSun, Coins, Bitcoin, Newspaper, GraduationCap } from "lucide-react";
...
async function getJson(url: string) { try { const r = await fetch(url, { next: { revalidate: 300 } }); return r.ok ? await r.json() : null; } catch { return null; } }
...
async function getSnapshot(): Promise<Snapshot[]> {
  ...
  const temp = weather?.current?.temperature_2m, feels = weather?.current?.apparent_temperature, inr = fx?.rates?.INR, btc = crypto?.bitcoin?.inr, btcChange = crypto?.bitcoin?.inr_24h_change;
  return [
...
async function getTrending() { const data = await getJson("https://news.google.com/rss/search?q=India%20when:1d&hl=en-IN&gl=IN&ceid=IN:en"); return data ? ["India today", "Technology", "Markets", "Sports", "Business", "World news"] : ["India today", "Technology", "Markets", "Sports", "Weather", "Bitcoin"]; }

const tools = [
  ["Police Study", "Daily SI & Constable preparation, quizzes and mock exams.", GraduationCap, "/study"],
...
export default async function Home() {
  ...
  return <main className="min-h-[calc(100vh-56px)] bg-white text-black">
    ...
    <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8"><div className="flex items-end justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[.2em] text-black/45">Live snapshot</p><h2 className="mt-1 text-2xl font-black tracking-tight">Right now</h2></div><span className="text-xs font-bold text-black/40">Auto-refresh · 5 min</span></div><div className="mt-4 grid gap-px overflow-hidden rounded-2xl border border-black/10 bg-black/10 sm:grid-cols-2 lg:grid-cols-4">{snapshot.map(({ label, value, detail, icon: Icon, href, live }) => <Link key={label} href={href} className="group bg-white p-5 transition hover:bg-black/[.025]"><div className="flex items-center gap-2"><Icon size={17}/><span className="text-xs font-bold text-black/50">{label}</span>{live&&<span className="ml-auto size-1.5 rounded-full bg-black" aria-label="Live data"/>}</div><p className="mt-5 text-2xl font-black tracking-tight">{value}</p><p className="mt-1 text-xs text-black/45">{detail}</p></Link>)}<LocalPriceSnapshot/></div></section>
