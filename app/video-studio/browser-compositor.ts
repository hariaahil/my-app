...
export type CompositionClip = { id: string; name: string; url: string; type: CompositionMediaType; duration: number; trimStart: number; trimEnd: number };
...
export async function exportBrowserComposition({ clips, ratio, text, mimeType, extension, maxDuration = 300 }: { clips: CompositionClip[]; ratio: CompositionRatio; text?: string; mimeType: string; extension: string; maxDuration?: number }) {
  ...
  await new Promise<void>((resolve) => { const render = () => { const elapsed = (performance.now() - startedAt) / 1000; if (elapsed >= duration) return resolve(); const position = compositionTimeAt(usable, elapsed); if (!position) return resolve(); const index = usable.findIndex((clip) => clip.id === position.clip.id); const source = media[index]; if (index !== previousIndex) { media.filter((item): item is HTMLMediaElement => item instanceof HTMLMediaElement).forEach((item) => item.pause()); audioNodes.forEach(({ gain }) => { gain.gain.value = 0; }); if (source instanceof HTMLMediaElement) source.play().catch(() => undefined); previousIndex = index; } audioNodes.forEach(({ media: audioMedia, gain }) => { const nodeIndex = media.indexOf(audioMedia); const clip = usable[nodeIndex]; if (!clip) return; gain.gain.value = nodeIndex === index ? audioFadeGain(position.localTime, clipLength(clip)) : 0; }); if (source instanceof HTMLVideoElement) { const sourceTime = Math.min(source.duration || position.clip.duration, position.clip.trimStart + position.localTime); if (Math.abs(source.currentTime - sourceTime) > 0.08) source.currentTime = sourceTime; fit(source, source.videoWidth || width, source.videoHeight || height); } else if (source instanceof HTMLImageElement) fit(source, source.naturalWidth || width, source.naturalHeight || height); else { ctx.fillStyle = "#000"; ctx.fillRect(0, 0, width, height); } if (text) { ctx.textAlign = "center"; ctx.font = "900 56px system-ui, sans-serif"; ctx.fillStyle = "white"; ctx.shadowColor = "rgba(0,0,0,.8)"; ctx.shadowBlur = 8; ctx.fillText(text, width / 2, height * 0.38, width - 80); ctx.shadowBlur = 0; } requestAnimationFrame(render); }; render(); });
